/**
 * Team Leader Cloud Functions
 * 专门处理 Team Leader 相关的点数分配、统计维护和警示检查
 * 
 * @version 2025-12-03
...
// ============================================================================
// 核心函数 1: onTeamLeaderAllocation
// ============================================================================

/**
 * 当 Team Leader 分配点数时自动触发
 ...
 * 5. 更新 teamLeaderStats
 * 6. 更新 Event.globalPointsStats
 * 7. 触发收款警示检查
 */
exports.onTeamLeaderAllocation = onDocumentCreated(
  {
  ...
  async (event) => {
    ...
    logger.info('[onTeamLeaderAllocation] 开始处理分配', {
    ...
    if (!allocation || !allocation.recipientId || !allocation.points) {
      logger.error('[onTeamLeaderAllocation] 分配数据不完整', allocation);
    ...
    try {
      ...
      logger.info('[onTeamLeaderAllocation] allocationType:', allocationType);

      const eventRef = db.doc(`organizations/${orgId}/events/${eventId}`);
      const eventDoc = await eventRef.get();
      
      if (!eventDoc.exists) {
        logger.error('[onTeamLeaderAllocation] Event 不存在', { eventId });
      ...
      if (allocation.points > maxPerAllocation) {
        logger.warn('[onTeamLeaderAllocation] 超出单次分配限额', {
      ...
      if (!recipientDoc.exists) {
        logger.error('[onTeamLeaderAllocation] 接收者不存在', {
          recipientId: allocation.recipientId
        });
      ...
      if (allocationType === 'inventory') {
        ...
        await recipientRef.update({
          ...
          [`seller.transactions.${timestampKey}`]: {
            ...
            allocatedBy:       'teamLeader',
            allocatedByUserId: smId,
            allocatedByName:   allocation.allocatedByName || 'Team Leader',
            allocatedByRole:   'teamLeader',
            note:              allocation.notes || 'Team Leader 库存分配',
            allocationDocId:   allocId
          },
          'pointsStats.totalReceived':              admin.firestore.FieldValue.increment(allocation.points),
          'pointsStats.receivedFromteamLeader':  admin.firestore.FieldValue.increment(allocation.points),
        ...
        logger.info('[onTeamLeaderAllocation] ✅ inventory: 更新 seller.availablePoints', {
          recipientId: allocation.recipientId, points: allocation.points
        });
      } else {
        // personal：HTTP 函数已完成 customer 点数写入，此处仅记录日志
        logger.info('[onTeamLeaderAllocation] personal: customer 点数已由 HTTP 函数写入，触发器跳过', {
          recipientId: allocation.recipientId, points: allocation.points
        });
      }
      ...
      batch.set(deptStatsRef, {
        ...
        'allocationStats.byteamLeader.count':           admin.firestore.FieldValue.increment(1),
        'allocationStats.byteamLeader.totalPoints':     admin.firestore.FieldValue.increment(allocation.points),
        lastUpdated: admin.firestore.FieldValue.serverTimestamp()
      }, { merge: true });

      // 3.2 更新 teamLeaderStats
      const smStatsRef = db.doc(
        `organizations/${orgId}/events/${eventId}/teamLeaderStats/${smId}`
      );
      ...
      if (allocationType === 'personal') {
        ...
        batch.update(eventRef, {
          'roleStats.teamLeaders.totalAllocations':      admin.firestore.FieldValue.increment(1),
          'roleStats.teamLeaders.totalPointsAllocated':  admin.firestore.FieldValue.increment(allocation.points),
      ...
      logger.info('[onTeamLeaderAllocation] ✅ 成功更新所有统计数据', { allocationType });

      // ========== 第4步: 异步触发收款警示检查 ==========
      // 注意：这里不等待，让它异步执行
      checkCollectionWarningForUser(db, orgId, eventId, allocation.recipientId, eventData)
        .catch(err => {
          logger.error('[onTeamLeaderAllocation] 收款警示检查失败', err);
        });

      return { success: true };

    } catch (error) {
      logger.error('[onTeamLeaderAllocation] ❌ 处理失败', {
        error: error.message,
        stack: error.stack,
        orgId, eventId, smId, allocId
      });
    ...
    try {
      ...
      transactions.forEach(tx => {
        ...
        if (tx.type === 'allocation') {
          ...
          } else if (tx.allocatedByRole === 'teamLeader') {
            stats.receivedFromteamLeader += tx.amount || 0;
          }
      ...
      // ========== 第3步: 传播到 Team Leader 统计 ==========
      const managedBy = afterData.managedBy || [];
      for (const smId of managedBy) {
        await updateTeamLeaderStatsFromUsers(db, orgId, eventId, smId);
      }
...
exports.checkCollectionWarnings = onDocumentWritten(
  ...
  async (event) => {
    ...
    try {
      ...
      const enableWarnings = eventData?.pointAllocationRules?.teamLeader?.enableWarnings !== false;
...
/**
 * 从用户数据重新计算 Team Leader 统计
 */
async function updateTeamLeaderStatsFromUsers(db, orgId, eventId, teamLeaderId) {
  const usersSnapshot = await db
    .collection(`organizations/${orgId}/events/${eventId}/users`)
  ...
  // 更新 Team Leader 统计文档
  await db.doc(`organizations/${orgId}/events/${eventId}/teamLeaderStats/${teamLeaderId}`).set({
    teamLeaderId,
  ...
  logger.info('[updateTeamLeaderStatsFromUsers] ✅ 更新 Team Leader 统计', {
    teamLeaderId,
    stats
  });
}
...
module.exports = {
  onTeamLeaderAllocation: exports.onTeamLeaderAllocation,
