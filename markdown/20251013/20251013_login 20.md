...
exports.loginWithPin = functions.https.onCall(async (data, context) => {
  try {
    ...
    // 驗證必填字段
    if (!phoneNumber || !pin) {
      throw new functions.https.HttpsError("invalid-argument", "請提供手機號碼與密碼");
    }
    if (!organizationId || !eventId) {
      throw new functions.https.HttpsError("invalid-argument", "請提供組織與活動信息");
    }
    ...
    // 🔥 修復：查詢時嘗試多種手機號格式
    const phoneVariants = [
      normalizedPhone,           // 例如: 123456789
      `0${normalizedPhone}`,     // 例如: 0123456789
      `60${normalizedPhone}`,    // 例如: 60123456789
      phoneNumber                // 原始輸入
    ];
    ...
    if (!passwordSalt || !storedHash) {
      throw new functions.https.HttpsError("failed-precondition", "用戶密碼資料不完整");
    }
    ...
    throw new functions.https.HttpsError(
      "internal", 
      error.message || "登入失敗，請稍後重試"
    );
  }
});

// 修復：changePassword 函數也需要類似的手機號處理
...
// 導出 Firestore 資料的 Cloud Function
  ...
  try {
    ...
    try {
      ...
      if (fnError.code === 'unavailable') {
        throw new Error('無法連接到服務器，請檢查網絡連接或稍後重試');
      }
