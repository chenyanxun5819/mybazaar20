...
const UserManagement = ({ organizationId, eventId, onClose, onUpdate }) => {
  ...
  const handleSaveRoles = async () => {
    ...
    try {
      ...
      const resp = await fetch('https://asia-southeast1-mybazaar-c4881.cloudfunctions.net/updateUserRoles', {
        method: 'POST',
        headers: {
  ...
  const handleAllocatePoints = async () => {
    ...
    try {
      ...
      const resp = await fetch('https://asia-southeast1-mybazaar-c4881.cloudfunctions.net/allocatePointsHttp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${idToken}`
  ...
  const handleRecallPoints = async () => {
    ...
    try {
      ...
      const resp = await fetch('https://asia-southeast1-mybazaar-c4881.cloudfunctions.net/recallPointsHttp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${idToken}`
  ...
  // 🆕 获取已被其他 Seller Manager 管理的部门列表（排除当前用户）
  const getManagedDepartmentsByOthers = () => {
    ...
    users.forEach(user => {
      ...
      // 如果用户有 sellerManager 角色，收集其管理的部门
      if (user.roles?.includes('sellerManager') && user.sellerManager?.managedDepartments) {
        user.sellerManager.managedDepartments.forEach(dept => {
          managedDepts.add(dept);
        });
      }
    });
    return managedDepts;
          ...
          <div style={styles.subModalContent}>
            ...
            {selectedRoles.sellerManager && (
              <div style={styles.managedDepartmentsSection}>
                <h4 style={styles.sectionTitle}>选择管理的部门</h4>
                <div style={styles.departmentsGrid}>
                  {(() => {
                    const managedByOthers = getManagedDepartmentsByOthers();
                    return departments.map(dept => {
                      const isAlreadyManaged = managedByOthers.has(dept);
                      const isSelected = managedDepartments.includes(dept);
                      return (
                        <label
                          key={dept}
                          style={{
                        ...
                        >
                          <input
                            ...
                            disabled={isAlreadyManaged && !isSelected}
                            style={styles.checkbox}
                          />
                          <span>
                            {dept}
                            {isAlreadyManaged && !isSelected && (
                              <span style={{ fontSize: '0.75rem', color: '#ef4444', marginLeft: '0.5rem' }}>
                                (已被管理)
                              </span>
                            )}
                          </span>
            ...
            <div style={styles.pointsForm}>
              ...
              <div style={styles.formGroup}>
                ...
                <div style={styles.identityTagsContainer}>
                  ...
                  {identityTags.map(tag => {
                    const count = getUserCountByIdentityTag(tag.id);
                    const zhName = tag.name['zh-CN'] || '';
                    const enName = tag.name['en'] || tag.name['en-US'] || '';
              ...
              {selectedIdentityTag && (
                <div style={styles.infoBox}>
                  ✅ 符合条件的用户: {getUserCountByIdentityTag(selectedIdentityTag)} 人
                </div>
              )}
              ...
              {/* 总计点数 */}
              {selectedIdentityTag && batchAmount && (
                <div style={styles.infoBox}>
                  总计需要: {(parseFloat(batchAmount) * getUserCountByIdentityTag(selectedIdentityTag)).toLocaleString()} 点数
                </div>
              )}
