 ...
 * ⭐ 修改版本（2026-01-17）：
 * 1. 移除待收点数列表（已移到 MerchantTransactions pending tab）
 * 2. 移除通知系统（已移到 MerchantDashboard 顶层）
 * 3. 保留 QR Code 生成、显示、下载、分享功能
 * 
 * @param {Object} props
  ...
  return (
    <div className="merchant-qr-container">
      ...
      {process.env.NODE_ENV === 'development' && (
        <details className="merchant-qr-debug">
          <summary>技术资讯</summary>
          <div className="merchant-qr-debug-content">
            <div>Merchant ID: {merchant.id}</div>
            <div>Event ID: {eventId}</div>
            <div>Org ID: {organizationId}</div>
            <div>User Role: {userRole}</div>
            <div>QR Data: {qrData.substring(0, 50)}...</div>
          </div>
        </details>
      )}
    </div>
