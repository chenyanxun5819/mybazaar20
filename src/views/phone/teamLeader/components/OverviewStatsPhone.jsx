import { useTeamLeaderStats, useManagedUsers } from '../../../../hooks/teamLeader';
...
const OverviewStatsPhone = ({
  ...
  const managedStats = {
    ...
    currentBalance: users.reduce((s, u) => s + (u.pointSeller?.availablePoints || 0), 0),
    totalRevenue: users.reduce((s, u) => s + (u.pointSeller?.totalRevenue || 0), 0),
    totalCollected: users.reduce((s, u) => s + (u.pointSeller?.totalCashCollected || 0), 0),
    pendingCollection: users.reduce((s, u) => s + (u.pointSeller?.pendingCollection || 0), 0),
    usersWithWarnings: users.filter(u => u.pointSeller?.collectionAlert?.hasWarning).length
  };
  ...
  const maxPerAllocation =
    eventData?.pointAllocationRules?.teamLeader?.maxPerAllocation ?? 100;
