...
const SellerManagerDashboard = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        console.log('  - sellerManager.managedDepartments:', userProfile.sellerManager?.managedDepartments);
  ...
  const loadSellers = () => {
    ...
    try {
      ...
      if (!Array.isArray(currentUser.managedDepartments) || currentUser.managedDepartments.length === 0) {
        ...
        console.warn('[loadSellers] 💡 这是正常的，如果您刚被创建为 Seller Manager 但还未分配部门');
        setSellers([]);
  ...
  const aggregateManagedUsersStats = (sellersList) => {
    ...
    sellersList.forEach(seller => {
      ...
      const sellerPoints = seller.seller || {};
