...
const SellerManagerTransactions = ({
  organizationId,
  eventId,
  sellerManagerId,
  managedDepartments = []
  ...
  useEffect(() => {
    if (!organizationId || !eventId || !sellerManagerId) {
      console.warn('[SellerManagerTransactions] 缺少必填参数');
      setLoading(false);
      return;
    }

    console.log('[SellerManagerTransactions] 开始监听分配记录', {
      organizationId,
      eventId,
      sellerManagerId
    });
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        console.log(`[SellerManagerTransactions] ✅ 查询到 ${snapshot.size} 条记录`);
      ...
      (err) => {
        console.error('[SellerManagerTransactions] ❌ 查询错误', err);
    ...
    // 清理函数
    return () => {
      console.log('[SellerManagerTransactions] 停止监听');
      unsubscribe();
    };
...
export default SellerManagerTransactions;
