...
const TeamLeaderTransactions = ({
  ...
  useEffect(() => {
    if (!organizationId || !eventId || !teamLeaderId) {
      console.warn('[TeamLeaderTransactions] 缺少必填参数');
      setLoading(false);
      return;
    }

    console.log('[TeamLeaderTransactions] 开始监听分配记录', {
      organizationId,
      eventId,
      teamLeaderId
    });
    ...
    const allocationsRef = collection(
      db,
      `organizations/${organizationId}/events/${eventId}/users/${teamLeaderId}/pointAllocations`
    );
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        console.log(`[TeamLeaderTransactions] ✅ 查询到 ${snapshot.size} 条记录`);
      ...
      (err) => {
        console.error('[TeamLeaderTransactions] ❌ 查询错误', err);
    ...
    // 清理函数
    return () => {
      console.log('[TeamLeaderTransactions] 停止监听');
      unsubscribe();
    };
...
export default TeamLeaderTransactions;
